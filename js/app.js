...
function detailsPopulate(data){
  ...
  movieSelection.innerHTML = "";
      ...
      movieSelection.innerHTML += "<div class=\"movieDetailPage small-9 small-centered column\"><h3>"+data[0].movies_title+"</h3><h4 class=\"movieYear\">"+data[0].movies_year+"</h4>"+data[0].movies_trailer+"<h4>CAST</h4></div>";
...
function detailsPopulateComment(data){
    for(i=0 ; i<data.length ; i++){
      movieSelection.innerHTML += "<div class=\"movieDetailComment small-9 small-centered column\"><h4>"+data[i].user_fname+" "+data[i].user_lname+"</h4><p>"+data[i].comment_content+"</p></div>";
      }

      movieSelection.innerHTML += "<input type=\"text\" name=\"comment\" placeholder=\"Place a comment here\" class=\"small-9 small-centered column\"></input><button class=\"comment button\">Submit</button>";
}
